...
public class DataRepository {
    ...
    public static void insertSong(Connection conn,
                                  int userId,
                                  String name,
                                  String artist,
                                  String album,
    ...
    public static Song selectUserSingleSong(Connection conn, int songId, int userId) throws SQLException {
        PreparedStatement stmt =
                conn.prepareStatement("SELECT * FROM songs s JOIN users u ON s.userId = u.id AND u.id = ? WHERE s.id = ?");
        stmt.setInt(1, userId);
        stmt.setInt(2, songId);
        ResultSet results = stmt.executeQuery();
        if (results.next()) {
            int id = results.getInt("songs.id"); // can't use the same aliases here. have to write out table name
    ...
    public static ArrayList<Song> selectUserSongs(Connection conn, String username) throws SQLException {
        ...
        PreparedStatement stmt =
                conn.prepareStatement("SELECT * FROM songs s JOIN users u ON s.userId = u.id AND u.name = ?");
    ...
    public static void editSong(Connection conn,
                                int userId,
                                int id,
                                String name,
                                String artist,
                                ...
                                int year) throws SQLException {
        PreparedStatement stmt =
                conn.prepareStatement("UPDATE songs SET name = ? , artist = ?, album = ?, genre = ?, year = ? WHERE id = ? AND userId = ?");
        stmt.setString(1, name);
    ...
    public static ArrayList<Song> searchSongName(Connection conn, String nameSearch) throws SQLException {
    ...
    // this was used just so i could test my other methods b/c I did not know how to pull out the songId. seems like not the right way to do it though.
    public static Song selectSongByName(Connection conn, String songName) throws SQLException {
        PreparedStatement stmt =
                conn.prepareStatement("SELECT * FROM songs WHERE name = ?");
